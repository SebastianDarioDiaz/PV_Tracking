...
public class RegistroTrackingController {
	...
	public class LocalidadController {
		...
		@RequestMapping("/registroTracking")
	...
	//agregar
		...
		@PostMapping("/formulario-2")
		public String crearRegistroTracking(@Valid @ModelAttribute("registroTrackingForm")RegistroTracking registroTracking,BindingResult result, ModelMap model) {
			...
			if(result.hasErrors()) {
				...
				model.addAttribute("registroTrackingForm",registroTracking);
				model.addAttribute("registroTracking", registroTrackingService.obtenerRegistroTrackings());
				model.addAttribute("formTab", "active");
		...
		@GetMapping("/formulario-2/cancelar")
		public String cancelarEditarRegistroTracking(ModelMap model) {
		...
		//modificar
		/*
		@GetMapping("/modificarRegistroTracking/{id}")
		public String obtenerFormularioModificarRegistroTracking(Model model,@PathVariable(name="id") Long id) throws Exception{
			try {
				RegistroTracking registroTrackingModificado = registroTrackingService.guardarRegistroTracking();// aqui va lo creado en localidad serviceimp y service
				model.addAttribute("registroTrackingForm", registroTrackingModificado);
			...
			model.addAttribute("listaRegistroTrackings", registroTrackingService.obtenerRegistroTrackings());
			model.addAttribute("formTab", "active");
			
		return "registroTrackingForm";
		}
		
		@PostMapping("/modificarRegistroTracking")
		public String postModificarRegistroTracking(@Valid @ModelAttribute("registroTrackingForm") RegistroTracking registroTracking, BindingResult result, ModelMap model) {
			if(result.hasErrors()) {
				...
				try {
					registroTrackingService.modificar(registroTracking);  // aqui va lo creado en localidad serviceimp y service
					model.addAttribute("registroTrackingForm", registroTracking);
					model.addAttribute("listTab", "active");
					model.addAttribute("editMode", "false");		
				} catch (Exception e) {
